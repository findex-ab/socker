function isObject(item: any): item is Record<PropertyKey, any> {
  return item !== null && typeof item === 'object' && !Array.isArray(item);
}

export function merge<T extends Record<PropertyKey, any>, U extends Record<PropertyKey, any>>(
  target: T,
  source: U
): T & U {
  for (const key of Object.keys(source) as (keyof U)[]) {
    ...
    // If both are plain objects, recurse
      ...
      if (isObject(tgtVal)) {
        ...
        (target as any)[key] = merge({}, srcVal);
      }
