...
(0, import_node_test.describe)("Dynamic buffer", () => {
  const buff = new DynamicBuffer();
  buff.writeString("hello");
  import_node_assert.default.strictEqual(buff.cursor, 5);
  buff.writeUint32(123);
  import_node_assert.default.strictEqual(buff.cursor, 9);
  buff.seek(0);
  import_node_assert.default.strictEqual(buff.cursor, 0);
  const msg = buff.readString(5);
  import_node_assert.default.strictEqual(msg, "hello");
  const num = buff.readUint32();
  import_node_assert.default.strictEqual(num, 123);
});
