...
(0, import_node_test.describe)("Stuff", () => {
...
(0, import_node_test.describe)("Transaction sanity test", () => {
  const trans = new Transaction({
    id: "my_transaction"
  });
  const chunks = [];
  ...
  if (gen) {
    ...
    for (const item of gen) {
      import_node_assert.default.strictEqual(item.getNumber("chunkIndex"), i);
      import_node_assert.default.strictEqual(item.getBytes("data") !== null, true);
      import_node_assert.default.strictEqual(item.getBytes("data") instanceof Uint8Array, true);
      const data = item.getString("data");
      import_node_assert.default.strictEqual(data, `data_${i}`);
      i++;
    }
  }
  trans.close();
